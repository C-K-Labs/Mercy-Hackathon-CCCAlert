...
function classifyIncident({
  ...
  const confidenceWeight = Math.round(conf * 26);
...
async function makeTwilioCall(classified) {
  ...
  await twilioClient.calls.create({
    ...
    twiml: `<Response><Say voice="alice">${message}</Say><Pause length="1"/></Response>`,
  });
...
function alertRowToIncident(row) {
  ...
  try { timeline = JSON.parse(row.timeline_json || '[]'); } catch (_) {}
  ...
  return {
    ...
    confidence: metadata.scoring ? clamp((metadata.scoring.confidenceWeight || 0) / 26, 0, 1) : 0.75,
...
app.post('/webhooks/camera-incidents', (req, res) => {
  ...
  const sourceKey = `${cameraId}-${timestamp}-${description}`;
...
app.get('/archived-incidents', (req, res) => {
  ...
  const incidents = rows.map((row) => {
    ...
    try { timeline = JSON.parse(row.timeline || '[]'); } catch (_) {}
    return {
      id: row.id,
